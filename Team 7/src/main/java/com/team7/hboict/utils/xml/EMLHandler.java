 ...
 * being processed.
 * <br/>
 * In case an unknown tag or attribute is being processed this will be reported through standard error
...
public class EMLHandler extends DefaultHandler implements TagAndAttributeNames {
    ...
    private CandidatesListTransformer candidatesListTransformer;
     ...
     *
     * @param candidatesListTransformer the <code>CandidateTransformer</code> that handles the transformation of the
     *                             provided data into the data model.
     */
    public EMLHandler(CandidatesListTransformer candidatesListTransformer) {
        this.candidatesListTransformer = candidatesListTransformer;
    }
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) {
        ...
        switch (localName) {
            case AFFILIATION_IDENTIFIER:
                electionData.remove(REGISTERED_NAME);
                break;
            case REGION:
            case COMMITTEE:
        ...
        switch (localName) {
            ...
            case CANDIDATE:
                ...
                if (candidatesListTransformer != null) {
                    savedElectionData.push(electionData);
                    electionData = new HashMap<>(electionData);
                }
                break;
            case LIST_DATA:
                candidatesListTransformer.registerParty(electionData);
                break;
    ...
    public void endElement(String uri, String localName, String qName) {
        ...
        // restored.
        switch (localName) {
            case ELECTION_IDENTIFIER:
                if (definitionTransformer != null) {
                    definitionTransformer.registerElection(electionData);
                }
                break;
            case REGION:
                if (registerRegion) {
            ...
            case CANDIDATE:
                // Ignore this when processing votes
                if (candidatesListTransformer != null) {
                    candidatesListTransformer.registerCandidate(electionData);
                    electionData = savedElectionData.pop();
                }
                break;
            case SELECTION:
